...
MotorControlSet motor_7(MOTOR_7, "NEW_MOTOR"); // 定义新增电机对象

// 添加 HdrarmControlSet 类定义
...
public:
    ...
    float arm_position_x, arm_position_y, arm_position_z;
    float arm_orientation_x, arm_orientation_y, arm_orientation_z, arm_orientation_w;
};

// 确保 HdrarmControlSet 对象正确声明
...
void can1_rx_Callback(can_msgs::Frame msg)
{
    ...
    switch (id_hex) {
        ...
        {
            motor_2.pos = motor_2.Uint_to_float(pos_hex, MOTOR_2_P_MIN, MOTOR_2_P_MAX, 16);
            motor_2.vel = motor_2.Uint_to_float(vel_hex, MOTOR_2_V_MIN, MOTOR_2_V_MAX, 12);
            motor_2.t   = motor_2.Uint_to_float(t_hex, MOTOR_2_T_MIN, MOTOR_2_T_MAX, 12);
            break;
        }
        ...
        case MOTOR_7 + MOTOR_ENABLED:
        {
            motor_7.pos = motor_7.Uint_to_float(pos_hex, motor_7.P_MIN, motor_7.P_MAX, 16);
            motor_7.vel = motor_7.Uint_to_float(vel_hex, motor_7.V_MIN, motor_7.V_MAX, 12);
            motor_7.t   = motor_7.Uint_to_float(t_hex, motor_7.T_MIN, motor_7.T_MAX, 12);
            break;
        }
        default:
        {
...
void motor_control_calculator(void)
{
    ...
    if(t_enable2now>=0.0 && t_enable2now<=0.1)//第一阶段，用0.3秒时间线性加入重力补偿
    {
    ...
    else if(t_enable2now>0.1 && arm_mode_last=="arm_init_finish_1")//第二阶段，回到零位
    {
        //这里虽然给的pos_tartget是一个阶跃，但是受motor_energy_limit()保护，不会產生危险
        motor_1.pos_target = 0;
        motor_2.pos_target = 0;
        motor_3.pos_target = 0;
        motor_4.pos_target = 0;
